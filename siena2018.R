...
# course information
COURSE_ID = "course-v1:TsinghuaX+10610193X+2017_T1"
INSTRUCTORS = c("ruby-cai")

# read data
COLUMNS = c("id", "create_time", "user_id", "user_name", "course_id", "parent_id")
data = read.csv("data/7-6621-encoded.csv", stringsAsFactors = FALSE, allowEscapes = TRUE)
table(data$course_id)
#data <- data[data$course_id == COURSE_ID, ]

data$user_id <- data$author_id
data$user_name <- data$author
data <- data[COLUMNS]
data[data$parent_id == 'NULL', 'parent_id'] <- NA
...
# waves
data$date_num = as.numeric(as.Date(data$create_time, format = "%Y-%m-%d"))
data$course_day = data$date_num - min(data$date_num) + 1
table(data$course_day)
...
print01Report(net_data, modelname = "descriptive")
